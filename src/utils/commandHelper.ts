...
const Routes = {
  ...
  guildCommand: (appId: string, guildId: string, cmdId: string): `/${string}` => {
 ...
 */
export async function deployCommands(folderPath: string, opts: DeployOptions): Promise<boolean> {
  ...
  const commandFiles = readdirSync(folderPath).filter((file) => file.endsWith(FILE_EXTENSION));

  if (opts.logs) console.log(`🔁 Started refreshing global and guild commands.`);
    ...
    for (const file of commandFiles) {
      ...
      try {
        ...
        if (typeof command != "object" || !("data" in command)) {
          ...
          if (opts.logs) console.log(`- Command '${command.data.name}' is ignored!`);
          continue;
        }
 ...
 */
export async function deleteCommand(commandId: string, opts: DeleteOptions): Promise<void> {
  const guildId = opts.guildId ?? null;

  const commandPath = guildId ? Routes.guildCommand(opts.appId, guildId, commandId) : Routes.command(opts.appId, commandId);

  if (commandId.match(/^\d+$/i)) {
    await new REST({ version: "10" }).setToken(opts.appToken).delete(commandPath);
  } else {
